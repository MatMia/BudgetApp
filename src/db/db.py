...
class LoginDB:
    ...
    def authenticate_login(self) -> bool:
    ...
    def add_new_user(self) -> bool:
        ...
        cur.execute('''INSERT INTO users VALUES (?,?)''', (self.username, self.password))
        con.commit()
        con.close()

        return self.authenticate_login()
    
    def validate_new_user_credentials(self) -> bool:
        if 6 < len(self.password) < 20:
            return True
        else:
            return False
        
    def check_user_existance(self) -> bool:
        con = sqlite3.connect('users.db')
        cur = con.cursor()
...
class BudgetDB:
    ...
    def modify_record(id,name,value,category,sub_category,expense_type,date):
        con = sqlite3.connect('budget.db')
        cur = con.cursor()

        cur.execute('''UPDATE budget
                    SET id = ?, name = ?, value = ?, category = ?, sub_category = ?, expense_type = ?, date = ?
                    WHERE id = ?''',
                      (id,name,value,category,sub_category,expense_type,date,id))
        con.commit()
        con.close()
...
class SubCategoriesDB:
    ...
    def show_db(**kwargs):
        ...
        elif "full_data" in kwargs:
            for row in cur.execute('''SELECT * FROM sub_categories'''):
                db_results.append(row)
...
class CategoriesAssignment:
    ...
    def show_db() -> []:
        con = sqlite3.connect('assignment_rules.db')
        cur = con.cursor()

        db_results = []

        for row in cur.execute('''SELECT * FROM assignment
        ORDER BY keyword'''):
            db_results.append(row)
    ...
    def insert_row(assignment_rule_uuid, keyword, type, category, sub_category) -> None:

        con = sqlite3.connect('assignment_rules.db')
        cur = con.cursor()
    
        cur.execute('''INSERT INTO assignment
        VALUES (?, ?, ?, ?, ?)''', (assignment_rule_uuid, keyword, type, category, sub_category))

        con.commit()
        con.close()

    def delete_record(assignment_rule_uuid) -> None:
        con = sqlite3.connect('assignment_rules.db')
        cur = con.cursor()

        cur.execute('''DELETE FROM assignment 
        WHERE assignment_rule_uuid = ?''', (assignment_rule_uuid,))
        con.commit()
        con.close()